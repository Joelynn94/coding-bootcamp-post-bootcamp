...
Your Answer here
The console log won't appear because we have not called the function yet
*/
...
function noPopUp() {
  alert('Okay, you won\'t get a pop noPopUp.')
}
...
function goodDay() {
  const userResponse = confirm('Are you having a nice day?')
  log(userResponse)
}
...
function userPopUp() {
    const answerPopup = confirm('Would you like a popup?')
    if(answerPopup) {
      popUp()
    } else {
      noPopUp()
